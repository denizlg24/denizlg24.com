...
export function BlogForm({ mode, blog, onSuccess, onCancel }: BlogFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "create") {
        const result = await client.post<{ message: string; blog: IBlog }>(
          "blogs",
          body,
        );
        ...
        const result = await client.patch<{ blog: IBlog }>(
          `blogs/${blog._id}`,
          body,
        );
