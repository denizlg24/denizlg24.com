...
interface GroupTreeComboboxProps {
  groups: INoteGroup[];
  value: string[];
...
export function GroupTreeCombobox({
  ...
  const visibility = useMemo(() => {
    ...
    const isVisible = (groupId: string): boolean => {
      ...
      for (const child of childrenByParent.get(groupId) ?? []) {
        if (isVisible(child._id)) descendantMatches = true;
      }
  ...
  const renderNode = (group: INoteGroup, depth: number): ReactNode => {
