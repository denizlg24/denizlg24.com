...
import type { IChatAttachment } from "./agent-types";
import { ChatInput, DEFAULT_MAX_ROUNDS } from "./chat-input";
import { ChatMessage } from "./chat-message";
import { mergeContentSegments } from "./chat-segments";
import {
  type StreamError,
  type StreamResult,
  useAgentStream,
...
export function ChatView({
  ...
  const { client, routes, slots } = useAdmin();
  ...
  useEffect(() => {
    if (
      ...
      onActiveRunChange?.(observedBackgroundRun);
      setBackgroundRunId(observedBackgroundRun.id);
      setActive(true);
    }
  }, [backgroundRunId, observedBackgroundRun, onActiveRunChange]);
  ...
  useEffect(() => {
    ...
    const refresh = async (): Promise<boolean> => {
      ...
      try {
        const result = await client.get<BackgroundAgentRunResponse>(
          `background-agent/runs/${backgroundRunId}`,
        );
        ...
        // A temporary network failure should not terminate background polling.
        return false;
      } finally {
  ...
  }, [backgroundRunId, client, onActiveRunChange]);

  useEffect(() => {
    if (!isActive || sidebarOpen) fetchConversations();
  }, [isActive, sidebarOpen, fetchConversations]);
  ...
  const loadConversation = async (meta: IConversationMeta) => {
    const result = await client
      .get<{ conversation: IConversation }>(`conversations/${meta._id}`)
      .catch(() => null);
    if (!result) {
  ...
  const deleteConversation = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await client.del<{ success: boolean }>(`conversations/${id}`);
  ...
  const handleMemoryModeChange = async (nextMode: AgentMemoryMode) => {
    ...
    const result = await client
      .patch<{ conversation: IConversation }>(
        `conversations/${conversationId}`,
        { memoryMode: nextMode },
      )
      .catch(() => null);
    if (!result) {
      toast.error("Failed to change memory mode");
      return;
    }
    setMemoryMode(result.conversation.memoryMode);
  };
  ...
  const uploadSingleAttachment = useCallback(
    async (att: IChatAttachment) => {
      ...
      const res = await client
        .upload<{ url: string; hash: string }>("upload", fd)
        .catch(() => null);
      if (!res) {
        setAttachments((prev) =>
          prev.map((a) =>
  ...
  const sendMessage = async (messageContent: string) => {
    if (isStreaming || !model || modelIncompatible) return;
    ...
    if (!currentConversationId) {
      ...
      const createResult = await client
        .post<{ conversation: IConversation }>("conversations", {
          title: msgTitle,
          model,
          memoryMode,
        })
    ...
    if (backgroundEnabled) {
      const result = await client
        .post<BackgroundAgentRunResponse>("background-agent/runs", {
  ...
  const continueChat = async (toolApprovals: Record<string, boolean>) => {
    if (isStreaming || !conversationId || !model) return;
    ...
    return (
      <>
        ...
        >
          ...
          {!isSheet && slots?.sidebarTrigger ? (
            <div className="absolute left-14 top-4">{slots.sidebarTrigger}</div>
          ) : null}
          <div
            className={
              isSheet
          ...
          >
            ...
            />
            {!isSheet && summary ? (
              <div className="mt-4 w-full min-w-0 max-w-3xl">{summary}</div>
            ) : null}
          </div>
        </div>
      </>
    ...
    <>
      ...
      <div className="relative flex h-full min-h-0 flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2 border-b px-2 py-2 sm:px-4">
          {!isSheet ? slots?.sidebarTrigger : null}
          {!(isSheet && sidebarOpen) ? (
            <Button
              variant="ghost"
              size="icon"
