...
import type { IEmail } from "@repo/schemas";
...
export function EmailList({
  ...
  const fetchEmails = useCallback(
    async (pageNum: number, searchQuery: string, append: boolean) => {
      ...
      try {
        const result = await client.get<EmailsResponse>(
          `email-accounts/${accountId}/emails?${params}`,
        );
        ...
        // ignore fetch errors; keep current list
      }
      setLoading(false);
      setLoadingMore(false);
      setSearching(false);
    },
    [client, accountId, emailListCache, emails],
  );
