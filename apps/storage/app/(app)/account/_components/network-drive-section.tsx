  ...
  return (
    <Tabs defaultValue="macos">
      ...
      <TabsContent value="windows" className="flex flex-col gap-3 pt-3">
        <Command
          label="Raise the 50 MB download cap — admin shell, run this first"
          value={
    ...
    <div className="flex flex-col gap-6">
      <dl className="grid grid-cols-[7rem_1fr] items-center gap-y-2 text-sm">
        ...
        <dd className="flex min-w-0 items-center gap-1">
          <span className="truncate font-mono text-xs">{DAV_URL}</span>
          <CopyButton value={DAV_URL} label="Copy WebDAV URL" />
        </dd>
        <dt className="text-muted-foreground">Username</dt>
        <dd className="truncate">{user.username}</dd>
        <dt className="text-muted-foreground">Password</dt>
        <dd className="text-muted-foreground">a credential issued below</dd>
      </dl>
