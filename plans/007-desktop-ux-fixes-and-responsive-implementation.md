...
| R5 | Web admin convergence on `@repo/ui` (doc §6 #011) | L | DONE (2026-06-13: admin dashboard shell now uses shared `PageHeader` + explicit scroll region; list/create/edit/detail routes use a compact `AdminPageHeader`; all compatible admin primitives moved to `@repo/ui` with only app-specific sidebar/sortable/time-picker left local; web `DataTable` promoted into `@repo/ui` with mobile toolbar/overflow and CSS column priority for contacts/comments. Gates: full turbo typecheck, desktop build (FULL TURBO), env-loaded web build (104 routes), all tests (140 web + 10 schemas + 10 utils + 1 desktop), Biome, and diff check green.) |
| R6 | Chat-home polish at small widths (doc §6 #012) | S | DONE (2026-06-13: chat history sheets use viewport-relative width below `sm` while preserving the 320px desktop panel; conversation model labels and docked input model label yield space on mobile; prompt text wraps without the fixed-height clipping trap; chat input/message padding tightens below `sm`; dashboard-summary loading skeleton wraps instead of overflowing. Browser probe at 375×812 + 1280×800: no document/visible-child overflow, no page errors, responsive drawer 307.5px/320px. Full typecheck, desktop+web builds, 161 tests, and Biome green.) |
