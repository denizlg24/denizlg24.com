...
| 015 | Comprehensive evolving personal-agent memory | P1 | XL | 014 | IN PROGRESS (2026-07-13: evidence/governance plus bounded conversation, note, calendar, person, project, course, journal and email-triage adapters; Gate B formation/embedding workers; Gate C hybrid shadow retrieval, traces, evaluation and shared inspection console landed on `feat/personal-agent-memory`. Live `mongot` index is READY/queryable and the exact filtered query passes; seven synthetic cases pass all initial Gate C metrics. All gates remain off and collections empty pending remaining Gate A adapters/deletion propagation, sequential A/B labelled samples and real shadow-trace review.) |
