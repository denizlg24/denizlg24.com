...
| 015 | Comprehensive evolving personal-agent memory | P1 | XL | 014 | IN PROGRESS (2026-07-13: all live Gates A-F released. Gate D injects bounded provenance-labelled memory only into enabled dashboard chat (owner trace `8f1f3601-a2d5-4e31-9306-096c6a53c357` beat the no-memory baseline at lower cost). Gate E reflection projects the evidence-backed user model with idempotent replay and audited rollback, bounded to 25 memories/job, daily at 04:00 UTC. Gate F ships a deterministic proactive insight engine (goal deadlines, calendar conflicts, stale follow-ups, contradictions, repeated failures) plus daily briefing into a shared web/desktop Inbox tab with Dismiss/Snooze/Useful feedback adaptation; released from a labelled five-insight live sample with three zero-write replay sweeps, per-category/day rate limits, 0.4-confidence abstention, prepare-only actions and external delivery off. Remaining: step 10 final evaluation (`eval:agent-memory`) and closing the done-criteria audit.) |
