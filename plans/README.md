...
| 015 | Comprehensive evolving personal-agent memory | P1 | XL | 014 | IN PROGRESS (2026-07-13: live Gates A/B released and Gate C enabled in non-injecting shadow mode; D/E/F remain off. Seven-domain backfill accepted 2,620 evidence events, Gate A passed its 50-event sample, Gate B reviewed six formed items (five safe promotions, one conflict queued), five live embeddings were written, and a real filtered vector retrieval returned five candidates with `injected:false`. Forced-tool formation, fair embedding scheduling, and transactional source-deletion cascades are committed on `feat/personal-agent-memory`; historical formation jobs are queued for the bounded worker and real owner-chat shadow traces still need labelling before Gate D.) |
