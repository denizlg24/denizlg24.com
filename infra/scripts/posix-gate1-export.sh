...
[[ -f "$root/samba/smb.conf" && ! -L "$root/samba/smb.conf" ]] && cp -a --no-dereference "$root/samba/smb.conf" "$work/snapshot/runtime/"
[[ -f "$root/samba/encryption-status.json" && ! -L "$root/samba/encryption-status.json" ]] && cp -a --no-dereference "$root/samba/encryption-status.json" "$work/snapshot/runtime/"
...
(cd "$work/snapshot" && find . -type f -print0 | sort -z | xargs -0 sha256sum > manifest.sha256)
