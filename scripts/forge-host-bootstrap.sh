...
. /etc/os-release
DISTRO="$ID"                  # ubuntu | debian — the vendor repos are separate
CODENAME="$VERSION_CODENAME"
ARCH="$(dpkg --print-architecture)"
echo "==> host is ${DISTRO} ${CODENAME} (${ARCH})"

# Vendor repositories lag new releases by months, and apt's error for a missing
# Release file names the codename but not the reason. Probe first, and fall
# back to the newest LTS that is actually published rather than failing on a
# host that is merely newer than the vendor.
resolve_codename() {
  local base="$1" want="$2"
  shift 2
  local candidate
  for candidate in "$want" "$@"; do
    ...
    if curl -fsS -o /dev/null "${base}/dists/${candidate}/Release"; then
      echo "$candidate"
      return 0
    fi
  done
...
apt-get install -y --no-install-recommends \
  ca-certificates curl gnupg git apt-transport-https

# --- docker -----------------------------------------------------------------
if ! command -v docker >/dev/null 2>&1; then
  echo "==> docker"
  docker_base="https://download.docker.com/linux/${DISTRO}"
  docker_codename="$(resolve_codename "$docker_base" "$CODENAME" noble jammy)" || {
    echo "docker publishes nothing for ${DISTRO}; install it by hand" >&2
    exit 1
  }
  [ "$docker_codename" = "$CODENAME" ] \
    || echo "    docker has no ${CODENAME}; using ${docker_codename}"
  install -m 0755 -d /etc/apt/keyrings
  curl -fsSL "${docker_base}/gpg" \
    | gpg --dearmor -o /etc/apt/keyrings/docker.gpg
  chmod a+r /etc/apt/keyrings/docker.gpg
  echo "deb [arch=${ARCH} signed-by=/etc/apt/keyrings/docker.gpg] \
${docker_base} ${docker_codename} stable" \
...
if ! command -v cloudflared >/dev/null 2>&1; then
  echo "==> cloudflared"
  cf_base="https://pkg.cloudflare.com/cloudflared"
  if cf_codename="$(resolve_codename "$cf_base" "$CODENAME" noble jammy)"; then
    curl -fsSL https://pkg.cloudflare.com/cloudflare-main.gpg \
      > /usr/share/keyrings/cloudflare-main.gpg
    echo "deb [signed-by=/usr/share/keyrings/cloudflare-main.gpg] \
${cf_base} ${cf_codename} main" \
      > /etc/apt/sources.list.d/cloudflared.list
    apt-get update -qq
    apt-get install -y cloudflared
  else
    # Cloudflare's apt repo covers fewer codenames than Docker's. The release
    # .deb has no codename in it at all, and cloudflared self-updates, so
    # nothing is lost by taking it directly.
    echo "    no apt repo for ${CODENAME}; installing the release .deb"
...
# --- CI deploy path ---------------------------------------------------------
# CI authenticates as the login user over Tailscale SSH and cannot type a sudo
# password. Rather than give that user blanket NOPASSWD — which is root, and
# so hands root to anything that compromises the CI tag — it may run exactly
# one root-owned script that it cannot edit.
DEPLOY_USER="${SUDO_USER:-denizlg24}"
echo "==> CI deploy path for ${DEPLOY_USER}"
mkdir -p /var/lib/forge-agent/staging
chown -R "${DEPLOY_USER}:${DEPLOY_USER}" /var/lib/forge-agent
chmod 0755 /var/lib/forge-agent
chmod 0775 /var/lib/forge-agent/staging

if [ -f /tmp/forge-agent-install ]; then
  install -o root -g root -m 0755 \
    /tmp/forge-agent-install /usr/local/sbin/forge-agent-install
fi

cat > /etc/sudoers.d/forge-agent <<SUDOERS
# Generated by forge-host-bootstrap.sh. One command, no arguments to smuggle.
${DEPLOY_USER} ALL=(root) NOPASSWD: /usr/local/sbin/forge-agent-install
SUDOERS
chmod 0440 /etc/sudoers.d/forge-agent
# A malformed drop-in locks everyone out of sudo. Validate, and remove it
# rather than leave a broken file behind.
visudo -cf /etc/sudoers.d/forge-agent >/dev/null \
  || { rm -f /etc/sudoers.d/forge-agent; echo "sudoers drop-in was invalid" >&2; exit 1; }
